...
public static class Typing
{
    ...
    public static bool TypeEqual(Context ctx, IType t1, IType t2)
    {
        ...
        return (t1s, t2s) switch
        {
            ...
            (TypeAll all1, TypeAll all2) =>
                TypeEqual(ctx.AddName(all1.Name), all1.Body, all2.Body),
            (TypeSome some1, TypeSome some2) =>
                TypeEqual(ctx.AddName(some1.Name), some1.Body, some2.Body),
    ...
    public static IType TypeOf(Context ctx, ITerm t)
    {
        switch (t)
        {
            ...
            case TAbs tabs:
                return new TypeAll(tabs.TypeVar, TypeOf(ctx.AddBinding(tabs.TypeVar, new TypeVarBind()), tabs.Body));
            case TApp tapp:
                var tTApp = SimplifyType(ctx, TypeOf(ctx, tapp.Term));
                if (tTApp is TypeAll all)
                    return Substitution.TypeSubsTop(tapp.TypeArg, all.Body);
            ...
            case Pack pack:
                ...
                if (simpPack is TypeSome some)
                {
                    ...
                    var tyU2 = Substitution.TypeSubsTop(pack.WitnessType, some.Body);
                    if (TypeEqual(ctx, tyU, tyU2))
                        return pack.ExistType;
            ...
            case Unpack unpack:
                var tyPack = SimplifyType(ctx, TypeOf(ctx, unpack.Package));
                if (tyPack is TypeSome some2)
                {
                    var ctxUnpack1 = ctx.AddBinding(unpack.TypeVar, new TypeVarBind());
                    var ctx2b = ctxUnpack1.AddBinding(unpack.V, new VarBind(some2.Body));
                    var tyBody = TypeOf(ctx2b, unpack.Body);
                    return Shifting.TypeShift(-2, tyBody);
                }
