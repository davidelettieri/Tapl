...
using static FullPoly.Core.Substitution;
...
public static class Evaluation
{
    ...
    private static bool IsVal(Context ctx, ITerm t) => t switch
    {
        ...
        Abs => true,
        TAbs => true,
        Pack pack when IsVal(ctx, pack.Term) => true,
    ...
    private static ITerm Eval1(Context ctx, ITerm t) => t switch
    {
        ...
        Unpack { Package: Pack pack } unpack when IsVal(ctx, pack.Term) =>
            TypeTermSubsTop(pack.WitnessType, TermSubsTop(TermShift(1, pack.Term), unpack.Body)),
        ...
        TApp { Term: TAbs tabs } tapp => TypeTermSubsTop(tapp.TypeArg, tabs.Body),
    ...
    private static ITerm Record(Context ctx, Record rec)
    {
        ...
        IEnumerable<(string, ITerm)> evalAField(List<(string, ITerm)> l)
        {
            if (l.Count == 0)
                throw new NoRulesAppliesException();
            ...
            if (IsVal(ctx, first.Item2))
                return Enumerable.Repeat(first, 1).Concat(evalAField(l[1..]));

            return Enumerable.Repeat((first.Item1, Eval1(ctx, first.Item2)), 1).Concat(l[1..]);
        }
    }
    ...
    private static ITerm CaseTag(IEnumerable<(string label, string variable, ITerm term)> cases, string label,
        ITerm term)
    {
