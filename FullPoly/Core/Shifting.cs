    ...
    public static IType TypeMap(Func<int, TypeVar, IType> onVar, int c, IType t)
    {
        IType Walk(int c, IType t)
        {
            return t switch
            {
                ...
                TypeId i => i,
                TypeString ts => ts,
                TypeSome some => new TypeSome(some.Name, Walk(c + 1, some.Body)),
                TypeUnit u => u,
                TypeRecord r => new TypeRecord(r.Variants.Select(p => (p.Item1, Walk(c, p.Item2)))),
                TypeFloat f => f,
                TypeBool b => b,
                TypeNat n => n,
                TypeAll all => new TypeAll(all.Name, Walk(c + 1, all.Body)),
        ...
        ITerm t)
    {
        ITerm Walk(int c, ITerm t)
        {
            return t switch
            {
                ...
                TAbs tabs => new TAbs(tabs.Info, tabs.TypeVar, Walk(c + 1, tabs.Body)),
