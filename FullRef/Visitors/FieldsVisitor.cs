...
public sealed class FieldsVisitor(TermVisitor termVisitor)
    : FullRefBaseVisitor<Func<(Context, int), IEnumerable<(string, ITerm)>>>
{
    private readonly FieldVisitor _fieldVisitor = new(termVisitor);

    public override Func<(Context, int), IEnumerable<(string, ITerm)>> VisitFields(FullRefParser.FieldsContext context)
    {
    ...
    public override Func<(Context, int), IEnumerable<(string, ITerm)>> VisitNefields_field(FullRefParser.Nefields_fieldContext context)
    {
    ...
    public override Func<(Context, int), IEnumerable<(string, ITerm)>> VisitNefields_field_comma_nefields(FullRefParser.Nefields_field_comma_nefieldsContext context)
    {
        var field = _fieldVisitor.Visit(context.field());
